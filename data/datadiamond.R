...
ip_per_game <- pitches %>%
  ...
  summarise(
    # Each of these outcomes records exactly one out against the pitcher
    outs_recorded = sum(outcome %in% c(
      "Strikeout Looking", "Strikeout Swinging",
      "Dropped Third Strike Looking", "Dropped Third Strike Swinging",
      "Groundout", "Flyout", "Popout", "Lineout",
      "Sacrifice Fly", "Sacrifice Bunt",
      "Error",           # batter reached but out is scored on the play
      "Truncated Out", "Caught Stealing", "Pickoff",
      "Batter Interference", "Additional Out"
    )),
    # DP/TP record additional outs beyond the batter's out
...
# PA-ending rows: exclude pitch-level outcomes and pure baserunning events
pa_final <- pitches %>%
...
pitcher_stats <- pitches %>%
  ...
  summarise(
    ...
    # Strikeouts: both standard and dropped third strike count as K for pitcher
    K  = sum(outcome %in% c(
      "Strikeout Looking", "Strikeout Swinging",
      "Dropped Third Strike Looking", "Dropped Third Strike Swinging"
    )),
...
write_json(pitches_json,
           "C:/Users/chris/OneDrive/Documents/pitches.json",
           auto_unbox = TRUE, pretty = TRUE)

write_json(pitcher_json,
           "C:/Users/chris/OneDrive/Documents/pitchers.json",
           auto_unbox = TRUE, pretty = TRUE, na = "null")

cat("Done!\n")
cat("pitches.json:", nrow(pitches_json), "batters\n")
cat("pitchers.json:", nrow(pitcher_json), "pitchers\n")
